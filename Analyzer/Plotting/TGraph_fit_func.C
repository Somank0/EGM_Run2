...
{   normalize=true;
    ...
    for(int i = 0; i < (int)hist.size(); i++) {
        ...
        latex.DrawLatex(0.8, 0.9-(0.15*i), Form("#mu = %.4f", fitFunc->GetParameter(0)));
        latex.DrawLatex(0.8, 0.85-(0.15*i), Form("#sigma_{L} = %.4f", fitFunc->GetParameter(1)));
        latex.DrawLatex(0.8, 0.8-(0.15*i), Form("#sigma_{R} = %.4f", fitFunc->GetParameter(2)));
...
void TGraph_fit_func(){
  ...
  int n_files=1;
    ...
    f[0] = new TFile("Plot_UL18_UL18.root");
    //f[0] = new TFile("Plot.root");
    
    vector<string> filetag=  {"Run 2"};
...
{"DRN_ErecoByEgen","ECorrByEGen","E_{Corrected}/E_{Gen}",200,0,1,0.95,1.05,{"DRN"}},
{"BDT_ErecoByEgen","ECorrByEGen","E_{Corrected}/E_{Gen}",200,0,1,0.95,1.05,{"BDT"}},
};
