...
class Electron_RefinedRecHit_NTuplizer : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   ...
   private:
      ...
      //noZS::EcalClusterLazyTools *clustertools_NoZS;
       std::unique_ptr<noZS::EcalClusterTools> clustertools_NoZS;
     // edm::ESHandle<EcalPedestals> _ped;
      ...
      // Electron variables
      int nElectrons_;
      std::vector<int>   GenMatch_;
      std::vector<float> DRN_corrected_E_;
      std::vector<float> DRN_correction_error__;
      Float_t rho;
      ...
      std::vector<float> Gen_ele_Pt;
      std::vector<float> Gen_ele_Eta;
      std::vector<float> Gen_ele_Phi;
      std::vector<float> Gen_ele_E;
      ...
      edm::Handle<edm::ValueMap<std::pair<float, float>>> gsfElectronsDRNHandle;
      //---------------- Input Tags-----------------------
      ...
      edm::ESGetToken<CaloGeometry, CaloGeometryRecord> pG;
      edm::ESGetToken<EcalPedestals, EcalPedestalsRcd> pedestalsToken_;
      edm::EDGetTokenT<edm::ValueMap<std::pair<float, float>>> gsfElectronsDRNToken_;
